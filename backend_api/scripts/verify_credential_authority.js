...
async function main() {
  ...
  try {
    ...
    if (instances.length > 0) {
      ...
      if (firstInstance) {
        ...
        const storedKey = whRes.data?.headers?.apikey;
  ...
  total++;
  if (webhookStoredKeyHash && processHash && webhookStoredKeyHash === processHash) {
    ...
    // Don't count as failure if we simply couldn't read it
  }
