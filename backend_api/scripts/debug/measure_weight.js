...
const searchInventorySchema = JSON.stringify({
...
- Sony WH-1000XM4: S/. 1200.00. Noise cancelling top. | Portada: https://s3.amazon.com/bucket/sony.jpg | Fotos (1): [https://s3.amazon.com/bucket/sony1.jpg]
- Baseus Encok: S/. 80.00. | Portada: Sin imagen
- Xiaomi Redmi Buds: S/. 60.00. | Portada: Sin imagen`;

function analyze(name, text) {
  const bytes = Buffer.byteLength(text, 'utf8');
  const chars = text.length;
  // Gemini en español promedia ~3.5 caracteres por token.
  const estimatedTokens = Math.ceil(chars / 3.5);
  console.log(`${name.padEnd(25)} | Bytes: ${bytes.toString().padEnd(5)} | Chars: ${chars.toString().padEnd(5)} | ~Tokens: ${estimatedTokens}`);
  return estimatedTokens;
}

console.log("=== ANÁLISIS DE PESO DE COMPONENTES DEL PROMPT ===\n");

let totalSysTokens = 0;
totalSysTokens += analyze("globalGuardrails", globalGuardrails);
totalSysTokens += analyze("systemCommands", systemCommands);
totalSysTokens += analyze("infoInstitucional", infoInstitucional);
totalSysTokens += analyze("mainInstructions", mainInstructions);
totalSysTokens += analyze("vision rule", vision);
console.log("-".repeat(60));
console.log(`TOTAL SYSTEM PROMPT (Est.): ~${totalSysTokens} tokens\n`);

analyze("search_inventory (Schema)", searchInventorySchema);
analyze("search_inventory (5 Results)", searchInventoryResult);
